...
activeClassName = 'active',
    // 当前激活弹窗classname
maskClassName = 'message-box__mask',
    // 遮罩层classname
defaultOption = {
  title: '提示', // 显示标题
  text: '', // 主体内容（文字）
...
function appendMask() {
  var node = mask.node = document.createElement('div');
...
function append(self) {
  ...
  node.innerHTML = '<div class="message-box__head">' + self.option.title + '<a href="javascript:void(0);" class="' + className + '__close" title="close/\u5173\u95ED">\xD7</a></div><div class="' + className + '__body">' + self.option.text + '</div><div class="' + className + '__foot"></div></div>';
  //绑定关闭事件
...
var Box = function () {
  function Box(option, events) {
    _classCallCheck(this, Box);

    this.id = counter++;
    this.option = Object.assign({}, defaultOption, typeof option === 'string' ? { option: option } : option);
    this.events = events;
  }
  /*
   * 激活当前弹窗到顶层
...
var AlertBox = function (_Box) {
  ...
  function AlertBox(option, events) {
    _classCallCheck(this, AlertBox);

    var _this = _possibleConstructorReturn(this, (AlertBox.__proto__ || Object.getPrototypeOf(AlertBox)).call(this, option, events));
...
var alert = exports.alert = function alert(option, events) {
